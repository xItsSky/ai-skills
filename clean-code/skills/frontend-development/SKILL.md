...
For TypeScript rules, the testing contract, git workflow, and security, follow the `core-development` skill. Next.js Server Actions and Route Handlers are backend surface: for those, also read the `backend-development` skill (`api-design.md`, `auth-security.md`).
